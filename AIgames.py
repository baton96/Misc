...
option = int(input('Select AI Algorithm: '))
if 0 < option < 9:
    depth = 4
    ...
    for p in range(2):
        ...
        if player == '1':  # user's turn
            while True:
                ...
                x, y = int(x), int(y)
                if ValidMove(board, x, y, player):
                    board, piecesTaken = MakeMove(board, x, y, player)
                    print('# of pieces taken: ' + str(piecesTaken))
                    break
                else:
                    print('Invalid move! Try again!')
        else:  # AI's turn
            x, y = BestMove(board, player, option)
            if not x == -1 and y == -1:
                board, piecesTaken = MakeMove(board, x, y, player)
